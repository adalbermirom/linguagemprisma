...
LUALIB_API int luaopen_utf8(lua_State *L) {
  luaL_Reg libs[] = {
#define ENTRY(name,cfunc) { #name, Lutf8_##cfunc }
    ENTRY(offset,offset),
    ENTRY(cods,codes),
    ENTRY(pontocod,codepoint),

    ENTRY(tamanho,len),
    ENTRY(corte,sub),
    ENTRY(inverta,reverse),
    ENTRY(minuscula,lower),
    ENTRY(maiuscula, upper),
    ENTRY(titulo,title),
    ENTRY(fold,fold),
    ENTRY(byte,byte),
    ENTRY(cod,byte),
    ...
    ENTRY(insira,insert),
    ENTRY(remova,remove),
    ENTRY(carpos,charpos),
    ENTRY(proximo,next),
    ENTRY(larg,width),
    ENTRY(largindex,widthindex),
    ENTRY(ncasecmp, ncasecmp),
    ENTRY(procure,find),
    ENTRY(capte,gmatch),
    ENTRY(troque,gsub),
    ENTRY(separe,match),
